...
const TH = { id: "여행", span: "all", sort: "idx", desc: true, all: false, cym: "" };

/* ---------- 소개팅 달력 ----------
   표로는 "누구에게 얼마" 가 보이지만 "언제 어떤 리듬으로 만났나" 가 안 보인다.
   달력에 놓으면 몰아친 달과 비어 있던 달, 무슨 요일에 만났나가 한눈에 보인다.
   이름과 년생만 칸에 넣고 나머지는 아래 목록에 둔다. 칸이 좁다. */
function introMeets() {
  const P = S.people || { intro: [], love: [] };
  /* 앱 과금 줄의 동행은 사람이 아니라 앱 이름이다. 사람 수에 섞이면 안 된다 */
  const apps = new Set((P.intro || []).filter(x => x.kind === "앱 과금").map(x => x.with || x.name));
  const out = [];
  for (const r of S.rows) {
    if (r.theme !== "소개팅") continue;
    const nm = (r.with || "").trim();
    if (!nm || apps.has(nm)) continue;
    out.push({ date: r.date, name: nm, mine: r.mine, place: r.place, no: r.no });
  }
  return out;
}
function introCalHTML() {
  const ms = introMeets();
  if (!ms.length) return "";
  const all = ms.map(m => m.date).sort();
  if (!TH.cym) TH.cym = ymOf(all[all.length - 1]);
  const ym = TH.cym;

  const byDay = new Map();
  ms.filter(m => ymOf(m.date) === ym).forEach(m => {
    const d = +m.date.slice(8, 10);
    if (!byDay.has(d)) byDay.set(d, []);
    byDay.get(d).push(m);
  });

  const y = +ym.slice(0, 4), mo = +ym.slice(5, 7);
  const first = new Date(y, mo - 1, 1), lastDay = new Date(y, mo, 0).getDate();
  let cells = "";
  for (let i = 0; i < first.getDay(); i++) cells += "<div></div>";
  for (let d = 1; d <= lastDay; d++) {
    const list = byDay.get(d) || [];
    const wd = new Date(y, mo - 1, d).getDay();
    const names = Array.from(new Set(list.map(x => x.name)));
    cells += '<div class="icd' + (list.length ? " has" : "") +
      (wd === 0 ? " sun" : wd === 6 ? " sat" : "") + '"' +
      (list.length ? ' data-icd="' + d + '"' : "") + ">" +
      '<span class="n">' + d + "</span>" +
      names.map(n => {
        const p = findPerson(n);
        return '<span class="who">' + esc(n) +
          (p && p.born ? '<i>' + esc(String(p.born).slice(-2)) + "</i>" : "") + "</span>";
      }).join("") + "</div>";
  }
  ...
  const rowsHTML = days.map(d => {
    ...
    return names.map(n => {
      const p = findPerson(n);
      const own = list.filter(x => x.name === n);
      const bits = [];
      if (p && p.born) bits.push(p.born + "년생");
      if (p && p.area) bits.push(p.area);
      if (p && p.job) bits.push(p.job);
      if (p && p.maker) bits.push(esc(p.route || "") + " " + esc(p.maker));
      return '<div class="row" data-icgo="' + own[0].no + '">' +
        '<div class="nm">' + esc(n) + '</div>' +
        '<div class="amt">' + won(own.reduce((a, x) => a + x.mine, 0)) + "</div>" +
        '<div class="meta"><span>' + date.slice(5) + " " + wdOf(date) + "요일</span>" +
        bits.map(b => "<span>" + b + "</span>").join("") +
        (own[0].place ? "<span>" + esc(own[0].place) + "</span>" : "") + "</div></div>";
    }).join("");
  }).join("");

  /* 무슨 요일에 만났나. 전체 기간 기준이라 달을 옮겨도 안 바뀐다 */
  const wdCnt = [0, 0, 0, 0, 0, 0, 0];
  const seen = new Set();
  ms.forEach(m => { const k = m.date + m.name; if (seen.has(k)) return; seen.add(k); wdCnt[new Date(m.date + "T00:00:00").getDay()]++; });
  const wdMax = Math.max(1, ...wdCnt);
  const best = WD[wdCnt.indexOf(wdMax)];

  const monthsWith = Array.from(new Set(ms.map(m => ymOf(m.date)))).sort();
  const at = monthsWith.indexOf(ym);
  const prev = at > 0 ? monthsWith[at - 1] : (monthsWith.filter(x => x < ym).pop() || "");
  const next = monthsWith.filter(x => x > ym)[0] || "";
  const nMon = byDay.size;

  return '<div class="card"><div class="sec"><h2>언제 만났나</h2>' +
    '<span class="hint">' + ms.length + "건 / " + monthsWith.length + "달</span></div>" +
    '<div class="icnav">' +
    '<button class="btn ghost sm" id="icPrev"' + (prev ? "" : " disabled") + ">지난 달</button>" +
    '<b>' + y + "년 " + mo + "월</b>" +
    '<button class="btn ghost sm" id="icNext"' + (next ? "" : " disabled") + ">다음 달</button>" +
    "</div>" +
    '<div class="icg">' + WD.map(w => '<div class="h">' + w + "</div>").join("") + cells + "</div>" +
    (nMon ? '<div class="rows" style="margin-top:12px">' + rowsHTML + "</div>"
          : '<p class="foot" style="margin:10px 0 0">이 달에는 만난 기록이 없습니다.</p>') +
    '<div style="margin-top:14px;border-top:1px solid var(--line2);padding-top:12px">' +
    '<p class="foot" style="margin:0 0 8px">무슨 요일에 만났나 (전체 기간)</p>' +
    barsHTML(WD.map((w, i) => ({ key: w, mine: wdCnt[i], color: i === 0 ? "var(--sky)" : i === 6 ? "var(--sky)" : "var(--coral)" })), { h: 100 }) +
    '<p class="foot">가장 많이 만난 요일은 <b>' + best + "요일</b>입니다. 줄을 누르면 그날 원장으로 갑니다.</p></div></div>";
}
function bindIntroCal(box) {
  const p = $("#icPrev", box), n = $("#icNext", box);
  const ms = introMeets();
  const months = Array.from(new Set(ms.map(m => ymOf(m.date)))).sort();
  const at = months.indexOf(TH.cym);
  if (p && !p.disabled) p.onclick = () => { TH.cym = months[Math.max(0, at - 1)]; renderTheme(); };
  if (n && !n.disabled) n.onclick = () => { TH.cym = months[Math.min(months.length - 1, at + 1)]; renderTheme(); };
  $$("[data-icgo]", box).forEach(el => el.onclick = () => jumpToRow(+el.dataset.icgo));
  $$("[data-icd]", box).forEach(el => el.onclick = () => {
    const d = +el.dataset.icd;
    const hit = ms.find(m => m.date === TH.cym + "-" + pad2(d));
    if (hit) jumpToRow(hit.no);
  });
}
...
/* ---------- 사람 목록 ----------
   기본은 최근 열다섯. 머리글을 누르면 그 칸으로 줄을 세우고, 한 번 더 누르면 뒤집는다. */
const PCOLS = [
  { k: "idx", t: "no", n: true, w: "" },
  { k: "name", t: "이름", n: false },
  { k: "route", t: "루트", n: false },
  { k: "maker", t: "주선자", n: false },
  { k: "first", t: "첫 만남", n: false },
  { k: "met", t: "만남", n: true },
  { k: "mine", t: "쓴 돈", n: true },
  { k: "per", t: "건당", n: true },
];
function peopleTable(rows, isIntro) {
  const cols = PCOLS.map(c => c.k === "met" && !isIntro ? { k: "days", t: "교제일", n: true } : c);
  const key = cols.some(c => c.k === TH.sort) ? TH.sort : "idx";
  const sorted = rows.slice().sort((a, b) => {
    const x = a[key], y = b[key];
    /* 빈칸은 줄 세우는 방향과 상관없이 늘 아래로 보낸다.
       오름차순일 때 빈칸이 맨 위에 몰리면 표가 쓸모없어진다 */
    const ex = x === "" || x == null, ey = y === "" || y == null;
    if (ex !== ey) return ex ? 1 : -1;
    const num = typeof x === "number" || typeof y === "number";
    let d = num ? (x || 0) - (y || 0) : String(x || "").localeCompare(String(y || ""), "ko");
    if (!d) d = (a.idx || 0) - (b.idx || 0);
    return TH.desc ? -d : d;
  });
  const CUT = 15;
  const show = TH.all ? sorted : sorted.slice(0, CUT);
  const arrow = c => c.k === key ? (TH.desc ? " 내림" : " 오름") : "";
  return '<div class="card"><div class="sec"><h2>사람별</h2>' +
    '<span class="hint">모두 ' + rows.length + "명" + (TH.all ? "" : " 중 " + show.length + "명") + "</span></div>" +
    '<p class="foot" style="margin:0 0 8px">머리글을 누르면 그 칸으로 줄을 세웁니다. 한 번 더 누르면 뒤집힙니다.</p>' +
    '<div style="overflow-x:auto"><table class="tbl ptbl"><thead><tr>' +
    cols.map(c => '<th class="sortable' + (c.k === key ? " on" : "") + '" data-pc="' + c.k + '">' +
      esc(c.t) + '<i>' + esc(arrow(c)) + "</i></th>").join("") + "</tr></thead><tbody>" +
    show.map(x =>
      "<tr>" + cols.map(c => {
        const v = x[c.k];
        if (c.k === "name") return '<td class="nm">' + esc(v || "") +
          ((x.next || "").indexOf("연애") >= 0 ? ' <span class="rfchip">연애</span>' : "") + "</td>";
        if (c.n) return '<td class="n">' + (v ? won(v) : (v === 0 ? "-" : "")) + "</td>";
        return "<td>" + esc(v || "") + "</td>";
      }).join("") + "</tr>").join("") +
    "</tbody></table></div>" +
    (rows.length > CUT
      ? '<button class="btn ghost sm" id="pMore" style="width:100%;margin-top:10px">' +
        (TH.all ? "접기" : "전체 " + rows.length + "명 보기") + "</button>"
      : "") +
    '<p class="foot">이름이 [입력] 시트의 동행 칸과 똑같아야 쓴 돈이 잡힙니다. 빈칸이면 이름을 맞춰 주세요.</p>' +
    '<button class="btn ghost sm" id="histSync" style="width:100%;margin-top:8px">집계 기간 다시 잡기</button>' +
    '<p class="foot" style="margin-top:6px">첫 만남과 연애 시작일과 가계부를 보고 기간을 규칙대로 다시 잡습니다. ' +
    '연애로 이어진 사람은 연애 시작 전날까지만 소개팅으로 셉니다. 같은 줄을 두 번 세지 않게 하려는 것입니다.</p></div>';
}

function bindTheme(box) {
  $$("[data-pc]", box).forEach(h => h.onclick = () => {
    const k = h.dataset.pc;
    if (TH.sort === k) TH.desc = !TH.desc;
    else { TH.sort = k; TH.desc = (k === "mine" || k === "per" || k === "met" || k === "days" || k === "idx"); }
    renderTheme();
  });
  bindIntroCal(box);
  const hs = $("#histSync", box);
  if (hs) hs.onclick = async () => {
    if (!await ensureAuth()) return;
    hs.disabled = true; hs.textContent = "다시 잡는 중";
    try {
      const j = await post("histSync", { token: AUTH.token });
      toast((j.sync && j.sync.msg) || "다시 잡았습니다", 5000);
      await loadAll(); renderAll();
    } catch (e) {
      hs.disabled = false; hs.textContent = "집계 기간 다시 잡기";
      toast("실패: " + e.message, 4000);
    }
  };
  const pm = $("#pMore", box);
  if (pm) pm.onclick = () => { TH.all = !TH.all; renderTheme(); };
...
function romanceHTML(which) {
  ...
  /* 6) 사람별. 이력에 적힌 사람을 하나도 빠뜨리지 않고 보여 준다.
        돈을 안 쓴 사람도 이력의 일부다. 걸러 내면 몇 명을 만났는지가 표에서 사라진다. */
  const rows = list.map(x => {
    const nm = x.with || x.name;
    const s = x.mine ? { mine: x.mine, n: x.cnt } : spentOf(nm);
    return { idx: x.idx || 0, name: nm, route: x.route, maker: x.maker,
             first: x.first || x.start, met: x.n, mine: s.mine, cnt: s.n,
             per: s.n ? Math.round(s.mine / s.n) : 0, next: x.next, days: x.days };
  });
  const tblCard = rows.length ? peopleTable(rows, isIntro) : "";

  const calCard = isIntro ? introCalHTML() : "";
  return hero + calCard + appCard + routeCard + makerCard + yearCard + meetCard + tblCard;
}
