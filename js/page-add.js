...
const ADD = { shots: [], cands: [], queue: [], qTotal: 0, sugg: null };
...
function openNewPerson(name, date, sheet) {
  ...
  openModal(
    ...
    " 을 [" + sn + "] 시트에 한 줄로 더합니다. 지출 집계는 동행 이름으로 저절로 걸립니다.</p>" +
    '<div class="form">' +
    '<div class="fl"><span>이름</span><input id="np_name" value="' + esc(name) + '"></div>' +
    '<div class="fl"><span>' + (love ? "만남 루트" : "소개 루트") + '</span><select id="np_route">' +
    INTRO_ROUTES.map(x => "<option>" + esc(x) + "</option>").join("") + "</select></div>" +
    '<div class="fl"><span>주선자</span><input id="np_maker" list="dl_maker" autocomplete="off" ' +
    'placeholder="지인 이름이나 앱 이름 (예: 수철, 틴더, 골드스푼)">' +
    '<datalist id="dl_maker">' + knownMakers().map(n => '<option value="' + esc(n) + '"></option>').join("") +
    "</datalist></div>" +
    '<div class="fl"><span>' + (love ? "연애 시작일" : "첫 만남") + '</span><input id="np_date" type="date" value="' + esc(date || todayISO()) + '"></div>' +
    "</div>" +
  ...
  $("#npSave").onclick = async () => {
    const nm = $("#np_name").value.trim();
    if (!nm) return toast("이름을 넣어 주세요");
    if (!await ensureAuth()) return;
    try {
      const j = await post("person", {
        token: AUTH.token, sheet: love ? "love" : "intro", name: nm,
        route: $("#np_route").value, maker: $("#np_maker").value.trim(), date: $("#np_date").value,
      });
      closeModal(true);
      toast(j.already ? "이미 있는 이름입니다" : "이력에 더했습니다");
      /* 이력만 늘었으므로 사람 목록만 손으로 채워 둔다 */
      const box = love ? S.people.love : S.people.intro;
      if (!j.already) box.push({ name: nm, with: nm, route: $("#np_route") ? "" : "", maker: "", first: "", n: 0 });
  ...
  box.innerHTML = '<div class="sec"><h2 style="font-size:13px">찾은 것 ' + ADD.cands.length + "건</h2>" +
    ...
    ADD.cands.map(function (c, i) {
      return '<div class="cand"><div class="t"><b>' + esc(c.place || "장소 모름") +
        ...
        '<div class="m">' + [c.date, c.time, c.pay, c.sub, c.detail].filter(Boolean).map(esc).join(" / ") + "</div>" +
