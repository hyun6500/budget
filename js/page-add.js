...
/* 결제수단 목록. 분류표에 아직 지역화폐가 없어도 고를 수 있게 앱이 한 값을 보탠다.
   시트 [분류표] 결제수단 칸에도 같은 이름으로 한 줄 더해 두어야 드롭다운이 맞습니다 */
function payList() {
  const M = S.meta || {};
  const a = (M.pays || []).slice();
  if (!a.some(isLocalPay)) a.push(LOCAL_PAY);
  return a;
}

/* ---------- 구성과 인원 ----------
   [누구 몫] 한 칸이 "누구와 나눴나" 와 "몇이서 나눴나" 두 질문을 같이 담고 있었다.
   고르는 칸을 둘로 가르고, 시트에 들어가는 이름은 분류표 사전에서 찾아 자동으로 채운다.
   사전이 아직 없는 시트(옛 분류표)에서는 예전처럼 이름 하나로만 고르게 둔다. */
function shareList() { return (S.meta && S.meta.shares) || []; }
function hasGroupDict() { return shareList().some(x => x.group); }
function curShare(v) {
  const s = (v && v.share) || "내 몫 전부";
  const hit = shareOf(s);
  return { name: s, group: (v && v.group) || (hit ? hit.group : ""),
           people: (v && v.people) || (hit ? hit.people : 1) };
}
function groupOptions(v) {
  const c = curShare(v);
  const gs = [];
  shareList().forEach(x => { if (x.group && gs.indexOf(x.group) < 0) gs.push(x.group); });
  if (!gs.length) return optList(shareList().map(x => x.name), c.name);
  return gs.map(g => '<option value="' + esc(g) + '"' + (g === c.group ? " selected" : "") + ">" +
    esc(g) + "</option>").join("");
}
function peopleOptions(v) {
  const c = curShare(v);
  const ns = peopleFor(c.group);
  return ns.map(n => '<option value="' + n + '"' + (Number(n) === Number(c.people) ? " selected" : "") + ">" +
    (n === 0 ? "남이 냄" : n + "명") + "</option>").join("");
}
/** 그 구성으로 고를 수 있는 인원들 */
function peopleFor(group) {
  const ns = [];
  shareList().forEach(x => { if (x.group === group && ns.indexOf(x.people) < 0) ns.push(x.people); });
  ns.sort((a, b) => a - b);
  return ns.length ? ns : [1];
}
/** 이름 하나로 구성과 인원 두 칸을 함께 맞춘다. 옛 코드가 [누구 몫] 만 알고 부를 때 쓴다 */
function setShare(R, name) {
  R = R || fscope();
  const out = $("#f_share", R), gsel = $("#f_group", R), psel = $("#f_people", R);
  if (!out || !gsel || !psel) return;
  if (!hasGroupDict()) { gsel.value = name || "내 몫 전부"; out.value = gsel.value; return; }
  const hit = shareOf(name || "내 몫 전부");
  if (!hit) return;
  gsel.value = hit.group;
  syncShare();
  psel.value = String(hit.people);
  syncShare();
}

/** 구성이나 인원을 고칠 때 나머지 한쪽과 [누구 몫] 을 맞춰 준다 */
function syncShare() {
  const R = fscope();
  const gsel = $("#f_group", R), psel = $("#f_people", R), out = $("#f_share", R);
  if (!gsel || !psel || !out) return;
  if (!hasGroupDict()) { out.value = gsel.value; return; }
  const ns = peopleFor(gsel.value);
  const want = Number(psel.value);
  psel.innerHTML = ns.map(n => '<option value="' + n + '"' + (n === want ? " selected" : "") + ">" +
    (n === 0 ? "남이 냄" : n + "명") + "</option>").join("");
  if (ns.indexOf(want) < 0) psel.value = String(ns[0]);
  out.value = shareName(gsel.value, Number(psel.value)) || "";
}
...
function formHTML(v) {
  ...
  return '<div class="form">' +
    ...
    '<div class="fl" id="fl_disc"><span>할인</span><input id="f_disc" inputmode="numeric" placeholder="0" value="' + (v.disc || "") + '"></div>' +
    "</div>" +
    /* 할인율은 시트에 칸을 만들지 않는다. 금액과 할인만 있으면 언제든 되짚을 수 있는 파생값이다.
       적을 때만 편하라고 두는 칸이라, 넣는 순간 할인 금액으로 바뀌어 들어간다 */
    '<div class="grid2">' +
    '<div class="fl" id="fl_discRate"><span>할인율<i>%</i></span>' +
    '<input id="f_discRate" inputmode="decimal" placeholder="예: 7" value="' + (v.discRate || "") + '"></div>' +
    '<div class="fl" id="fl_discBy"><span>할인수단</span><input id="f_discBy" list="dl_discBy" autocomplete="off" ' +
    'placeholder="쿠팡 포인트, 서울사랑페이" value="' + esc(v.discBy || "") + '">' +
    '<datalist id="dl_discBy">' + knownDiscBy().map(n => '<option value="' + esc(n) + '"></option>').join("") +
    "</datalist></div></div>" +
    '<div style="font-size:11px;color:var(--ink3);margin-top:-6px" id="f_realHint"></div>' +
    '<div class="grid2">' +
    ...
    '<div class="grid2">' +
    '<div class="fl" id="fl_group"><span>구성<i>누구와</i></span><select id="f_group">' +
    groupOptions(v) + "</select></div>" +
    '<div class="fl" id="fl_people"><span>인원<i>몇이서</i></span><select id="f_people">' +
    peopleOptions(v) + "</select></div>" +
    "</div>" +
    '<div class="grid2">' +
    '<div class="fl" id="fl_pay"><span>결제수단</span><select id="f_pay">' + optList(payList(), v.pay) + "</select></div>" +
    '<div class="fl" id="fl_share"><span>누구 몫<i>저절로</i></span>' +
    '<input id="f_share" readonly tabindex="-1" value="' + esc(v.share || "내 몫 전부") + '"></div>' +
    "</div>" +
    '<div class="grid2">' +
...
function readForm() {
  ...
  return {
    ...
    share: g("#f_share") || g("#f_group"), group: g("#f_group"), people: numOf(g("#f_people")),
    pay: g("#f_pay"), situ: g("#f_situ"),
...
function validate(v) {
  ...
  if (!v.share) return "[구성] 과 [인원] 을 맞춰 고를 수 없습니다. 분류표에 그 조합이 없습니다";
  const R = ruleCheck(v);
...
function bindBigHint() {
  ...
  const gp = $("#f_group", R), pp2 = $("#f_people", R);
  if (gp) gp.onchange = () => { syncShare(); paintRules(); };
  if (pp2) pp2.onchange = () => { syncShare(); paintRules(); };
  ["#f_situ", "#f_treat", "#f_theme", "#f_once", "#f_time"].forEach(id => {
  ...
  const dc = $("#f_disc", R); if (dc) dc.oninput = () => { syncRateFromDisc(); paintRules(); };
  const rt = $("#f_discRate", R); if (rt) rt.oninput = () => { syncDiscFromRate(); paintRules(); };
  const at = $("#f_amt", R); if (at) at.oninput = () => { syncDiscFromRate(); paintReal(); };
  const py = $("#f_pay", R); if (py) py.onchange = () => { paintReal(); paintRules(); };
  ...
  syncShare();
  syncRateFromDisc();     /* 고치기 창을 열 때 이미 적힌 할인에서 율을 되짚어 보여 준다 */
  f();
}

/* ---------- 할인과 할인율 ----------
   시트에 들어가는 것은 언제나 할인 금액이다. 할인율은 적을 때만 쓰는 임시 칸이라
   치는 쪽을 살려 다른 쪽을 다시 그린다. 서로를 되받아 치지 않도록 자물쇠를 둔다. */
let DISC_BUSY = false;
function syncDiscFromRate() {
  if (DISC_BUSY) return;
  const R = fscope();
  const amt = numOf($("#f_amt", R) ? $("#f_amt", R).value : "");
  const rt = $("#f_discRate", R), dc = $("#f_disc", R);
  if (!rt || !dc) return;
  const p = parseFloat(String(rt.value).replace(/[^0-9.]/g, ""));
  DISC_BUSY = true;
  if (rt.value === "" || !isFinite(p)) { /* 율을 지우면 금액은 그대로 둔다 */ }
  else if (amt > 0) dc.value = String(Math.round(amt * Math.min(100, Math.max(0, p)) / 100));
  DISC_BUSY = false;
  paintReal();
}
function syncRateFromDisc() {
  if (DISC_BUSY) return;
  const R = fscope();
  const amt = numOf($("#f_amt", R) ? $("#f_amt", R).value : "");
  const rt = $("#f_discRate", R), dc = $("#f_disc", R);
  if (!rt || !dc) return;
  const d = numOf(dc.value);
  DISC_BUSY = true;
  rt.value = (amt > 0 && d > 0) ? String(Math.round(d / amt * 1000) / 10) : "";
  DISC_BUSY = false;
  paintReal();
}
...
function paintReal() {
  ...
  const pay = $("#f_pay", R) ? $("#f_pay", R).value : "";
  if (amt < 0) {
  ...
  if (!disc) {
    el.innerHTML = isLocalPay(pay)
      ? '<b style="color:var(--gold)">지역화폐는 액면가보다 싸게 삽니다.</b> 깎인 만큼을 할인이나 할인율에 적어 주세요. 7%% 면 할인율 칸에 7 만 넣으시면 됩니다.'.replace("%%", "%")
      : "";
    return;
  }
  const real = Math.max(0, amt - disc);
  const pct = amt > 0 ? Math.round(disc / amt * 1000) / 10 : 0;
  el.innerHTML = "영수증 " + won(amt) + "원 중 " + won(disc) + "원" +
    (pct ? " (" + pct + "%)" : "") + "을 할인받아 <b>실제 결제 " +
    won(real) + "원</b>입니다. 통계에는 실제 결제액이 들어갑니다. 시트에는 할인 금액으로 들어갑니다.";
}
...
function paintDemands(v, chk) {
  ...
  ["treat", "with", "event", "disc"].forEach(f => {
    const fl = $("#fl_" + f, R);
    if (!fl) return;
...
function openNewPerson(name, date, sheet) {
  ...
  $("#npSave").onclick = async () => {
    ...
    try {
      const send = force => post("person", {
        token: AUTH.token, sheet: love ? "love" : "intro", force: !!force,
      ...
      /* 같은 이름이 이미 있으면 덮어쓰지 않고 물어본다.
         지윤 네 줄처럼 동명이인이 실제로 있고, 합치면 쓴 돈이 섞인다 */
      if (j.already) {
        const who = (j.same || []).map(x => [x.maker, x.first].filter(Boolean).join(" ")).join(" / ");
        const go = confirm(g("#np_name") + " 은 이미 이력에 있습니다.\n" +
          (who ? "먼저 적힌 것: " + who + "\n\n" : "\n") +
          "동명이인이면 [확인] 을 눌러 새 줄로 더합니다.\n같은 사람이면 [취소] 를 누르세요.");
        if (!go) { closeModal(true); return; }
        j = await send(true);
      }
      closeModal(true);
      toast(j.sync && j.sync.intro ? "이력에 더하고 집계 기간을 잡았습니다" : "이력에 더했습니다");
      const box = love ? S.people.love : S.people.intro;
      box.push(j.person || { name: g("#np_name"), with: g("#np_name"), route: g("#np_route"),
                             maker: g("#np_maker"), job: g("#np_job"), area: g("#np_area"),
                             born: g("#np_born"), first: g("#np_date"), n: 1, mine: 0, cnt: 0 });
      cacheSave(); paintRules();
...
function clearForm() {
  const R = fscope();
  ["#f_amt", "#f_place", "#f_detail", "#f_with", "#f_memo", "#f_time", "#f_disc", "#f_discRate", "#f_discBy"]
...
/* ---------- 지난 줄을 그대로 베껴 새로 적기 ----------
   수도세처럼 격월로 같은 내용을 적는 것이 많다. 날짜만 바꿔 다시 적게 한다.
   no 와 검수는 베끼지 않는다. 그 둘은 줄마다 새로 매겨지는 것이다. */
function copyToAdd(r, date) {
  goTab("add");
  setTimeout(() => {
    clearForm();
    const set = (id, val) => { const n = $(id); if (n && val != null && val !== "") n.value = val; };
    set("#f_date", date || nextDateFor(r));
    set("#f_amt", r.amt);
    set("#f_place", r.place);
    set("#f_detail", r.detail);
    set("#f_sub", r.sub);
    setShare(fscope(), r.share);
    set("#f_pay", r.pay);
    set("#f_situ", r.situ);
    set("#f_treat", r.treat);
    set("#f_theme", r.theme);
    ...
    set("#f_time", r.time);
    set("#f_memo", r.memo);
    /* 할인은 베끼지 않는다. 지난번에 쿠폰을 썼다고 이번에도 쓰는 것이 아니다 */
    const R = fscope();
    paintSubs(R);
    if ($("#f_sub", R)) $("#f_sub", R).value = r.sub || "";
    syncBig(R);
    bindBigHint();
    paintRules();
    const a = $("#f_amt", R);
    if (a) { a.focus(); a.select && a.select(); }
    toast("지난 줄을 베꼈습니다. 날짜와 금액을 보고 고치세요");
  }, 60);
}

/** 같은 곳에서 되풀이해 적은 간격을 보고 다음 날짜를 짚는다.
    수도세가 두 달에 한 번이면 마지막 납부일 + 두 달을 내민다. */
function nextDateFor(r) {
  const key = (r.sub || "") + "|" + (r.place || "").trim();
  const ds = S.rows
    .filter(x => (x.sub || "") + "|" + (x.place || "").trim() === key)
    .map(x => x.date).sort();
  if (ds.length < 2) return todayISO();
  const gaps = [];
  for (let i = 1; i < ds.length; i++)
    gaps.push(Math.round((new Date(ds[i]) - new Date(ds[i - 1])) / 86400000));
  gaps.sort((a, b) => a - b);
  const med = gaps[Math.floor(gaps.length / 2)];
  if (!med || med < 20 || med > 400) return todayISO();     /* 되풀이로 보기 어려우면 오늘 */
  const last = new Date(ds[ds.length - 1] + "T00:00:00");
  last.setDate(last.getDate() + med);
  const next = last.getFullYear() + "-" + pad2(last.getMonth() + 1) + "-" + pad2(last.getDate());
  /* 너무 먼 앞날이면 오늘이 낫다 */
  const far = new Date(todayISO()); far.setDate(far.getDate() + 21);
  return next > (far.getFullYear() + "-" + pad2(far.getMonth() + 1) + "-" + pad2(far.getDate()))
    ? todayISO() : next;
}
    ...
    ADD.cands.map(function (c, i) {
      ...
      return '<div class="cand' + (c.done ? " off" : "") + '"><div class="t"><b>' + esc(c.place || "장소 모름") +
        ...
        (dup && dup.length
          ...
          : "") +
        /* 무엇을 근거로 그렇게 분류했는지 보여 준다. 추정은 근거를 남긴다 */
        (c.why ? '<div class="m" style="color:var(--sky)">' + esc(c.why) +
          (c.subWas ? " (판독은 " + esc(c.subWas) + " 라고 했습니다)" : "") + "</div>" : "") +
        '<div class="act">' +
...
function openEdit(r) {
  openModal(
    ...
    formHTML({
      ...
      group: r.group, people: r.people,
    }) +
    '<div style="display:flex;gap:8px;margin-top:16px">' +
    '<button class="btn" id="edSave">고치기</button>' +
    '<button class="btn ghost" id="edRefund">환불 적기</button></div>' +
    '<div style="display:flex;gap:8px;margin-top:8px">' +
    '<button class="btn ghost sm" id="edCopy" style="width:100%">이 줄로 새로 적기</button>' +
  ...
  $("#edCopy").onclick = () => { closeModal(true); copyToAdd(r); };
