...
function formHTML(v) {
  ...
  return '<div class="form">' +
    ...
    "</div>" +
    '<div class="grid2">' +
    '<div class="fl" id="fl_with"><span>동행</span><input id="f_with" value="' + esc(v.with || "") + '"></div>' +
    '<div class="fl"><span>메모</span><input id="f_memo" value="' + esc(v.memo || "") + '"></div>' +
    "</div>" +
    '<div id="f_rules"></div></div>';
}
...
function paintRules() {
  ...
  if (!chk.hit.length) { box.innerHTML = ""; return; }
  box.innerHTML = chk.hit.map(r => {
