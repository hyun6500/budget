...
function renderFame() {
  ...
  /* 숨은 절약 */
  const sv = all.filter(r => SAVE_RE.test((r.memo || "") + " " + (r.detail || "")));
  let svSum = 0;
  for (const r of sv) {
  ...
  const svCard = sv.length
    ? '<div class="card"><div class="sec"><h2>숨은 절약</h2><span class="hint">메모와 세부내역에서 찾은 것</span></div>' +
      ...
      '<div class="row"><div class="nm">메모에 적힌 금액의 합</div><div class="amt">' + won(svSum) + "</div></div></div>" +
      '<div style="height:10px"></div>' +
      barsHTML(svY.map(y => ({ key: y.key.slice(2), mine: y.n, color: "var(--in)" })), { h: 100 }) +
      '<p class="foot">메모에 숫자를 적어 둔 것만 더한 값이라 실제 아낀 돈보다 적습니다. 막대는 건수입니다.</p></div>'
    : "";
