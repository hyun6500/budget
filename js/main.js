...
const RENDER = { month: renderMonth, ledger: renderLedger, spend: renderSpend, asset: renderAsset, hall: renderHall, add: renderAdd, search: renderSearch };
const TITLE = { month: "이번 달", ledger: "원장", spend: "소비", asset: "자산", hall: "기록실", add: "입력", search: "찾기" };
const DIRTY = { month: 1, ledger: 1, spend: 1, asset: 1, hall: 1, add: 1, search: 1 };
const TABS = ["month", "ledger", "spend", "asset", "hall", "add", "search"];
const BARTABS = ["month", "ledger", "spend", "asset", "hall"];
let LASTTAB = "month";   /* 입력이나 찾기를 닫으면 돌아갈 곳 */

const NAV = { skip: false };
function goTab(t, viaPop) {
  if ((t === "add" || t === "search") && S.tab !== t) LASTTAB = S.tab;
  ...
  $("#fab").hidden = (t === "add" || t === "search");
  if (t === "search") DIRTY.search = 1;
...
/* ---------- 찾은 줄로 건너가기 ----------
   찾기에서 줄을 누르면 그 줄이 있는 달로 원장을 옮기고, 표를 그린 뒤에 그 줄을 짚어 준다.
   빈 날을 접어 둔 상태거나 다른 달을 보고 있으면 아무리 스크롤해도 없는 줄이라
   달을 먼저 맞추고 접힘을 푼 다음에 찾는다. */
function jumpToRow(no) {
  const r = S.rows.find(x => x.no === +no);
  if (!r) return toast("그 줄을 찾지 못했습니다");
  if (S.ym !== r.ym) {
    S.ym = r.ym;
    DIRTY.month = DIRTY.ledger = DIRTY.spend = DIRTY.hall = 1;
  }
  if (typeof LG !== "undefined") LG.hideEmpty = false;
  DIRTY.ledger = 1;
  goTab("ledger");
  paintTitle();
  setTimeout(() => {
    const tr = $('#p-ledger tr[data-no="' + no + '"]');
    if (!tr) { toast("원장에서 그 줄을 못 찾았습니다"); return; }
    try { tr.scrollIntoView({ block: "center", behavior: "smooth" }); } catch (e) { tr.scrollIntoView(); }
    tr.classList.add("hit");
    clearTimeout(jumpToRow._t);
    jumpToRow._t = setTimeout(() => tr.classList.remove("hit"), 4000);
  }, 90);
}
...
async function boot() {
  ...
  try {
    ...
    /* 담아 둔 것으로 이미 그려 놓았으면 화면을 망치지 않는다.
       다만 띠가 [새로 읽는 중] 인 채로 굳어 있으면 안 되므로 무슨 일인지 적어 준다 */
    if (painted) { failStale(e); toast("새 자료를 못 받았습니다. 지난번 것을 보고 있습니다"); return; }
...
/** 새로 읽기에 실패했을 때. 띠를 [읽는 중] 으로 남겨 두지 않는다 */
function failStale(e) {
  const el = $("#staleBar");
  if (!el) return;
  el.classList.add("bad");
  el.innerHTML = '<span class="dot"></span>지난번 자료를 보고 있습니다. ' +
    esc(String(e && e.message || "새로 읽지 못했습니다")) +
    ' <button id="staleGo" type="button">다시 읽기</button>';
  el.hidden = false;
  const b = $("#staleGo");
  if (b) b.onclick = () => { el.classList.remove("bad"); el.innerHTML = '<span class="dot"></span>새로 읽는 중입니다.'; refetch(); };
}

/** 다시 읽기. 갱신 단추와 띠의 단추가 함께 쓴다 */
async function refetch() {
  try {
    await loadAll(true);
    paintTabs(); renderAll(); stamp(); hideStale();
    toast("최신입니다");
  } catch (e) {
    failStale(e);
    toast("못 받았습니다: " + String(e && e.message || e));
  }
}
...
$("#findBtn").onclick = () => goTab("search");
$("#reloadBtn").onclick = async () => {
  toast("다시 읽는 중");
  A.loaded = false; A.denied = false; A.data = null;
  /* 예전에는 여기에 try 가 없어서, 못 읽으면 아무 말도 없이 [다시 읽는 중] 에서 멈췄다 */
  await refetch();
};
