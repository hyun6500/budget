...
function tripWho(rows) {
  ...
  if (sh.some(x => /^가족/.test(x))) return "가족";
  if (sh.some(x => /^연인/.test(x))) return "연인";
  if (sh.some(x => /^동생/.test(x))) return "동생";
  if (sh.some(x => /^지인/.test(x))) return "지인";
