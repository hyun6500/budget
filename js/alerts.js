...
function buildAlerts(ym) {
  ...
  /* 10. 규칙에 어긋난 줄 */
  /* 여행은 이제 소분류를 얽매지 않는다. 테마와 소분류는 서로 다른 질문에 답한다 */
  const broke = S.rows.filter(r => {
    if ((r.sub === "용돈" || r.sub === "선물" || r.sub === "여행 선물") && !r.treat) return true;
    if (r.theme === "데이트" && r.treat !== "연인") return true;
    if (r.theme === "보은" && !r.treat) return true;
    if (r.theme === "소개팅" && !r.with) return true;
    return false;
  });
  if (broke.length) out.push({
    key: "broke:" + broke.length, tag: "규칙",
    html: "예외 규칙과 어긋난 줄이 <b>" + broke.length + "건</b> 있습니다. " +
      "(용돈과 선물에 대접이 비었거나, 데이트인데 대접이 연인이 아니거나, 소개팅인데 동행이 비었거나)",
  });
