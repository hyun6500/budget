...
/* 지역화폐 결제수단. 서울사랑, 마포사랑처럼 자치구마다 이름이 갈리지만
   장부에서는 값 하나로 묶는다. 축을 나누는 것은 결제수단이 아니라 할인수단이다. */
const LOCAL_PAY = "서울사랑페이";
function isLocalPay(v) { return /사랑(페이|상품권)|지역화폐/.test(String(v || "")); }
...
const RULES = [
  ...
  {
    key: "반반", title: "나눠 낸 것",
    how: "[금액] 은 전액 그대로 + [누구 몫] 에서 누구와 몇이서 나눴는지 고르기",
    note: "미리 나눠서 적지 마세요. 전액을 두고 비율로 나눠야 나중에 비율이 바뀌어도 되돌릴 수 있습니다. " +
      "[누구 몫] 은 구성(가족 연인 동생 지인)과 인원(둘 셋 넷 다섯)을 합친 이름입니다. " +
      "부담률은 1 나누기 인원이고 [내 몫] 은 저절로 계산됩니다.",
    on: v => (Number(v.people) > 1) || /반반|둘이서|셋이서|넷이서|다섯이서/.test(v.share || ""),
    need: v => ((v.group === "연인" || v.group === "지인" ||
                 /^연인/.test(v.share || "") || /^지인/.test(v.share || "")) && !v.with)
      ? { field: "with", msg: "누구와 나눠 냈는지 [동행] 에 이름을 적어 주세요" } : null,
  },
  {
    key: "가족여행", title: "가족이 함께 다녀온 여행",
    how: "[금액] 전액 + [누구 몫] 가족 둘이서부터 다섯이서까지 + 테마 [여행]",
    note: "가족계금으로 다녀온 여행입니다. 결제는 한 사람이 해도 부담은 나눠 집니다. 몇이서 나눴는지에 따라 값을 고르세요.",
    on: v => v.group === "가족" || /^가족/.test(v.share || ""),
    need: () => null,
  },
  {
  ...
  {
    key: "지역페이", title: "서울사랑페이 같은 지역화폐",
    how: "[결제수단] 서울사랑페이 + [할인] 에 깎인 만큼. 할인율을 넣으면 금액이 저절로 계산됩니다",
    note: "서울사랑상품권은 늘 액면가보다 싸게 삽니다. 10만원권을 7% 깎아 93,000원에 샀다면 " +
      "영수증은 10만원이지만 실제로 나간 돈은 93,000원입니다. 할인을 안 적으면 그만큼 더 쓴 것으로 잡힙니다. " +
      "마포사랑 같은 자치구 상품권도 같은 값 하나로 묶습니다. 어느 구 것인지는 [할인수단] 에 적으세요.",
    on: v => isLocalPay(v.pay),
    fill: v => (!v.discBy && v.disc > 0 ? { discBy: "서울사랑페이" } : null),
    need: v => ((v.amt || 0) > 0 && !(v.disc > 0))
      ? { field: "disc", msg: "지역화폐는 늘 싸게 삽니다. [할인] 이나 [할인율] 에 깎인 만큼을 적어 주세요" } : null,
  },
  {
    key: "남이낸것", title: "남이 낸 것",
    how: "[누구 몫] 을 남이 낸 것으로 (부담률 0)",
    note: "기록은 남기되 내 지출에서는 빠집니다. 상대가 사준 밥을 남겨 두고 싶을 때 씁니다.",
    on: v => v.share === "남이 낸 것" || v.group === "남" || Number(v.people) === 0,
    need: () => null,
  },
];
...
const FIELD_NAME = { treat: "대접", with: "동행", event: "건", sub: "소분류", amt: "금액", detail: "세부내역", disc: "할인" };
function ruleDemands(v) {
  ...
  for (const r of RULES) {
    ...
    for (const f of ["treat", "with", "event", "disc"]) {
      let n = null;
