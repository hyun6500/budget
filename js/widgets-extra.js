...
function heatHTML(map, nDays) {
  ...
  /* 돌려받은 돈이 더 큰 날은 붉은 눈금이 아니라 제이드로 칠한다.
     그냥 v <= t1 로 흘려보내면 가장 옅은 칸이 되어 '조금 쓴 날'처럼 보인다 */
  const shade = v => v < 0 ? "rgba(87,214,169,.55)"
    : !v ? "var(--soft)" : v <= t1 ? "rgba(179,58,90,.20)" : v <= t2 ? "rgba(179,58,90,.42)"
    : v <= t3 ? "rgba(179,58,90,.68)" : "var(--out)";
  const cells = vals.map(x =>
    '<i style="background:' + shade(x.v) + '" title="' + x.k + " " +
    (x.v ? (x.v < 0 ? "돌려받음 " + won(-x.v) + "원" : won(x.v) + "원") : "기록 없음") + '"></i>').join("");
...
function barsHTML(pts, opt) {
  ...
  /* 예전에는 Math.abs 로 음수도 위로 그렸다. 그러면 돌려받은 달이 많이 쓴 달처럼 보인다.
     이제 0 선을 두고 음수는 아래로 내려 그린다 */
  const maxV = Math.max(1, ...pts.map(p => p.mine || 0));
  const minV = Math.min(0, ...pts.map(p => p.mine || 0));
  const span = maxV - minV;
  const plot = H - ptop - pb;
  const zero = ptop + plot * (maxV / span);
  const iw = (W - pl - pr) / pts.length;
  let s = '<svg class="spark" viewBox="0 0 ' + W + " " + H + '" preserveAspectRatio="none" role="img">';
  [.5, 1].forEach(g => { const y = ptop + plot * (1 - g); s += '<line class="grid" x1="0" y1="' + y + '" x2="' + W + '" y2="' + y + '"/>'; });
  if (minV < 0) s += '<line class="grid" x1="0" y1="' + zero.toFixed(1) + '" x2="' + W + '" y2="' + zero.toFixed(1) + '" stroke="var(--ink3)"/>';
  pts.forEach((p, i) => {
    const v = p.mine || 0;
    const h = plot * (Math.abs(v) / span);
    s += '<rect x="' + (pl + i * iw + iw * .2).toFixed(1) + '" y="' + (v >= 0 ? zero - h : zero).toFixed(1) +
      '" width="' + (iw * .6).toFixed(1) + '" height="' + Math.max(0, h).toFixed(1) +
      '" rx="1.6" fill="' + (v < 0 ? "var(--in)" : (p.color || o.color || "var(--out)")) +
      '" opacity="' + (p.hi ? 1 : .85) + '"/>';
  });
