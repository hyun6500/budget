...
function suggestFor(place, amt) {
  ...
  /* 환불 줄(음수)은 배움의 재료에서 뺀다. 보통 얼마를 쓰는 곳인지가 흐려진다 */
  let hits = S.rows.filter(r => r.amt > 0 && normPlace(r.place) === key);
  if (!hits.length) {
    /* 똑같지 않으면 앞뒤로 품은 이름까지 찾아본다 */
    hits = S.rows.filter(r => {
      if (r.amt <= 0) return false;
...
/* 리뉴얼로 뷔페와 도시락은 식당기타로 합쳐졌고, 편의점이 끼니 자리에 들어왔다 */
const MEAL_SUBS = /^(구내식당|치킨|피자\/버거|분식|한식|고기\/구이|일식|중식|양식|아시안|식당기타|배달앱|카페|베이커리|디저트|편의점|주점|유흥)$/;
...
function applySuggest(sg, scope, ctx) {
  ...
  /* 누구 몫은 칸이 둘로 갈렸다. 이름 하나로 두 칸을 함께 맞춘다 */
  if (sg.share && sg.share.share >= 0.4 && sg.share.v && sg.share.v !== "내 몫 전부") {
    const g0 = $("#f_group", R);
    if (g0 && typeof setShare === "function") {
      setShare(R, sg.share.v);
      done.push("누구 몫 " + sg.share.v + " (" + Math.round(sg.share.share * 100) + "%)");
    }
  }
