...
ledger/
  ...
  js/
    config.js           개인 설정. 채울 곳은 APPS_SCRIPT_URL 하나뿐입니다
    core.js             불러오기, 줄 만들기, 달 집계, 기기 캐시, 그 자리 반영
    ...
    rules.js            예외 열두 가지와 사람 되짚기
    alerts.js           홈 알림 열 갈래와 숨기기
    suggest.js          지난 기록에서 배우는 추천, 시각으로 끼니 짚기
    ...
    page-theme.js       기록실 > 테마, 소개팅 인포그래픽
    story.js            월말 결산 카드
    page-refund.js      환불과 지우기. 원래 줄은 고치지 않고 음수 줄을 새로 적습니다
    page-add.js         입력, 메일에서 찾기, 후보 다루기
    main.js             켜기, 탭 옮기기, 뒤로가기, 가리기와 밤낮
  apps-script/
    Code.gs             서버 전체. 가계부 시트의 Code.gs 에 통째로 붙여 넣습니다
  설치안내.md            처음부터 세우는 순서
  규칙과_정책.md          이 프로젝트의 판단 기준 전부
  예외입력_규칙.md        예외 열두 가지와 주선자 적는 곳
  이어가기_프롬프트.md     새 대화에 붙여 넣을 글
  README.md
```
...
- **환불은 원래 줄을 고치지 않습니다.** 돌려받은 날에 금액이 음수인 지출 줄을 새로 적습니다. 구분을 새로 만들거나 수입으로 넣지 않습니다. 그래서 화면 조각들이 음수를 견뎌야 합니다. `donut` 은 음수 조각을 원에서 빼고 아래에 적어 주고, `barsHTML` 과 `stackHTML` 과 `sparkMonths` 는 0 선을 두고 아래로 그립니다. 새 그래프를 만들 때 이 점을 잊지 마세요
